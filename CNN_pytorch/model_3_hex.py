...
def save_hex(tensor, filename, width):
    with open(f"hardware/model_3/{filename}", "w") as f:
...
sd = torch.load("models/fashion_model_3.pth", weights_only=True)
...
W_b5, B_b5 = get_folded_params('conv_b5.0', 'conv_b5.1', sd, B_SCALE[4])
save_hex(W_b5, "conv_b5_w.hex", W_LENGTH)
save_hex(B_b5, "conv_b5_b.hex", 32)
W_b6, B_b6 = get_folded_params('conv_b6.0', 'conv_b6.1', sd, B_SCALE[5])
save_hex(W_b6, "conv_b6_w.hex", W_LENGTH)
save_hex(B_b6, "conv_b6_b.hex", 32)
...
with torch.inference_mode():
    ...
    x = F.conv2d(x.double(), W_b5.double(), B_b5.double(), padding=1)
    print(f"B5 - Max after Conv: {x.max().item()}, Min: {x.min().item()}")
    x = x.to(torch.int32) >> Q_SHIFT[4]
    x = torch.clamp(F.relu(x), 0, MAX_VAL)
    print(f"B5 - Max after Shift/ReLU: {x.max().item()}, Min: {x.min().item()}")
    save_hex(x.to(torch.int32), "golden_b5.hex", W_LENGTH)
    
    x = F.conv2d(x.double(), W_b6.double(), B_b6.double(), padding=1)
    print(f"B6 - Max after Conv: {x.max().item()}, Min: {x.min().item()}")
    x = x.to(torch.int32) >> Q_SHIFT[5]
    x = torch.clamp(F.relu(x), 0, MAX_VAL)
    print(f"B6 - Max after Shift/ReLU: {x.max().item()}, Min: {x.min().item()}")
    save_hex(x.to(torch.int32), "golden_b6.hex", W_LENGTH)
...
B_cls_q = torch.round(B_cls * B_SCALE[6]).to(torch.int32)
